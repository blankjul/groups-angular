...
export class ConstraintsComponent extends AbstractComponent {
  ...
  public onClick(member: Member) {
    if (this.groupToAdd.has(member)) { this.groupToAdd.delete(member); } else { this.groupToAdd.add(member); }
  }
  ...
  public groups() {
    if (this.inGroup) { return this.problem.inOneGroup; } else { return this.problem.notInOneGroup; }
  }

  hoverClass() {
    if (this.inGroup) { return 'success'; } else {  return 'danger'; }
  }
